...
export default function Admin() {
  ...
  const [cvSubTab, setCvSubTab] = useState('experiences');
          ...
          >
                ...
                {currentList.map((item, i) => (
                  <div key={i} className="flex flex-col sm:flex-row items-center justify-between p-4 rounded-2xl border border-slate-200 hover:border-indigo-300 hover:shadow-md transition-all gap-4">
                    <div className="flex items-center gap-4 w-full sm:w-auto">
                      { (item.imageUrl || item.coverImage || item.img) ? (
                        <div className="w-16 h-16 rounded-xl bg-slate-100 flex-shrink-0 border border-slate-200 overflow-hidden">
                           <img src={item.imageUrl || item.coverImage || item.img} alt={item.title} className="w-full h-full object-cover" />
                        </div>
                      ) : (
                         <div className="w-16 h-16 rounded-xl bg-slate-100 flex-shrink-0 border border-slate-200 flex items-center justify-center text-slate-400">No Img</div>
                      ) }
                      <div>
                        <h4 className="font-bold text-slate-900 text-lg">{item.title || item.name || item.text || 'Untitled'}</h4>
                        <p className="text-sm font-medium text-slate-500">{item.category || item.author || 'Uncategorized'}</p>
                      </div>
                    </div>
            ...
            {activeTab === 'CV' && (
              <div>
                <div className="flex space-x-2 mb-6 overflow-x-auto pb-2">
                   {['experiences', 'education', 'skills', 'certificates'].map(sub => (
                      <button 
                        key={sub}
                  ...
                  {currentList.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row items-center justify-between p-4 rounded-2xl border border-slate-200 hover:border-indigo-300 hover:shadow-md transition-all gap-4">
                      <div className="w-full sm:w-auto">
                        <h4 className="font-bold text-slate-900 text-lg">{item.role || item.degree || item.name}</h4>
                        <p className="text-sm font-medium text-slate-500">{item.company || item.institution || item.category || item.issuer}</p>
                      </div>
                      <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                        <button onClick={() => openModal(item)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"><Edit2 className="w-4 h-4" /></button>
                        <button onClick={() => handleDelete(cvSubTab, item.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </div>
                  ))}
                  {currentList.length === 0 && <div className="text-slate-500 text-center py-10 text-sm font-medium">No items found in {cvSubTab}. Add one above.</div>}
                </div>
              </div>
            )}
            ...
            {activeTab === 'Settings' && (
              <div className="space-y-8 max-w-3xl">
                <div>
                  <h3 className="text-lg font-bold text-slate-900 border-b border-slate-200 pb-2 mb-6">Personal Info (CV)</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input type="text" className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" defaultValue="https://api.dicebear.com/7.x/notionists/svg?seed=NsameReneTamjong&backgroundColor=e2e8f0" />
                    </div>
                    <div className="md:col-span-2">
              ...
              >
                <div className="p-6 md:p-8">
                  <div className="flex items-center justify-between mb-8">
                    <h3 className="text-2xl font-bold text-slate-900">{editingItem ? 'Edit' : 'Add New'} {activeTab === 'CV' ? cvSubTab : activeTab.replace(/s$/, '')}</h3>
                  ...
                  <form onSubmit={handleSave} className="space-y-5">
                    {/* Dynamic Fields based on activeTab/cvSubTab */}
                    {(activeTab === 'Projects' || activeTab === 'Blogs' || activeTab === 'Courses' || activeTab === 'Gallery') && (
                      <>
                        <div>
                        ...
                        {activeTab === 'Projects' && (
                           <div>
                             <label className="block text-sm font-semibold text-slate-700 mb-2">Live Review Link (Optional)</label>
                             <input name="liveDemoLink" type="text" defaultValue={editingItem?.liveDemoLink || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                           </div>
                        )}
                        {(activeTab === 'Projects' || activeTab === 'Blogs' || activeTab === 'Courses') && (
                          <div>
                    ...
                    {activeTab === 'Quotes' && (
                      <>
                        <div>
                          ...
                          <textarea name="text" rows={4} required defaultValue={editingItem?.text || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none resize-none text-sm font-medium" />
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Author</label>
                          <input name="author" type="text" required defaultValue={editingItem?.author || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                        </div>
                      </>
                    )}

                    {activeTab === 'CV' && cvSubTab === 'experiences' && (
                      <>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Company</label>
                          <input name="company" type="text" required defaultValue={editingItem?.company || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Role</label>
                          <input name="role" type="text" required defaultValue={editingItem?.role || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                        </div>
                        <div className="flex gap-4">
                           <div className="flex-1">
                             <label className="block text-sm font-semibold text-slate-700 mb-2">Start Date</label>
                             <input name="startDate" type="text" required defaultValue={editingItem?.startDate || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                           </div>
                           <div className="flex-1">
                             <label className="block text-sm font-semibold text-slate-700 mb-2">End Date</label>
                             <input name="endDate" type="text" defaultValue={editingItem?.endDate || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                           </div>
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Description</label>
                          <textarea name="description" rows={4} required defaultValue={editingItem?.description || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none resize-none text-sm font-medium" />
                        </div>
                      </>
                    )}

                    {activeTab === 'CV' && cvSubTab === 'education' && (
                      <>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Institution</label>
                          <input name="institution" type="text" required defaultValue={editingItem?.institution || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Degree/Field of Study</label>
                          <input name="degree" type="text" required defaultValue={editingItem?.degree || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                        </div>
                        <div className="flex gap-4">
                           <div className="flex-1">
                             <label className="block text-sm font-semibold text-slate-700 mb-2">Start Date</label>
                    ...
                    {activeTab === 'CV' && cvSubTab === 'skills' && (
                      <>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Skill Name</label>
                          <input name="name" type="text" required defaultValue={editingItem?.name || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Category (e.g. Frontend, Backend)</label>
                          <input name="category" type="text" required defaultValue={editingItem?.category || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Proficiency (0-100)</label>
                          <input name="proficiency" type="number" min="0" max="100" required defaultValue={editingItem?.proficiency || 80} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                        </div>
                      </>
                    )}

                    {activeTab === 'CV' && cvSubTab === 'certificates' && (
                      <>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Certificate Name</label>
                          <input name="name" type="text" required defaultValue={editingItem?.name || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Issuer</label>
                          <input name="issuer" type="text" required defaultValue={editingItem?.issuer || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Date</label>
                          <input name="date" type="text" required defaultValue={editingItem?.date || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 mb-2">Link</label>
                          <input name="link" type="text" defaultValue={editingItem?.link || ''} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-indigo-600 outline-none text-sm font-medium" />
                        </div>
                      </>
                    )}
